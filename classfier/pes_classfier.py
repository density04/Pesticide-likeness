...
def pes_classifer(filepath):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    pos_data=list(open("data/pes1.txt"))
    ...
    gcn_model.load_state_dict(torch.load('models/para/gcn_pes.pth'))
    ...
    rnn_model.load_state_dict(torch.load('models/para/rnn_pes.pth'))
    ...
    DCAN_model.load_state_dict(torch.load('models/para/dcan_pes.pth'))
    ...
    rnn_train_min=22.84301818673941
    rnn_train_max=94.04658239605106
    gcn_train_min=0.0001658791152294
    gcn_train_max=0.9999992847442628
    dcan_train_max=1
    dcan_train_min=0
